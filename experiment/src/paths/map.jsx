...
const NearbyHospitalsMap = () => {
  ...
  function updatehos(hospitalName) {
    setHospitals((prevHospitals) => {
      // Create a set of unique hospital names
      const uniqueHospitals = new Set(prevHospitals.map(hospital => hospital[0][1]));

      // Check if hospitalName is not already present in the set
      if (!uniqueHospitals.has(hospitalName[0][1])) {
        // Add hospitalName only if it's not already present
        return [...prevHospitals, hospitalName];
      }

      // If hospitalName is already present, return the current state
    ...
    function successLocation(position) {
      ...
      fetch(apiUrl)
        ...
        .then((data) => {
          ...
          const hospitalsApiUrl = `https://nominatim.openstreetmap.org/search?format=json&limit=20&q=hospital&addressdetails=1&viewbox=${
            longitude - 1.5
          },${latitude - 1.5},${longitude + 1.5},${latitude + 1.5}`;
        ...
        .then((data) => {
          data.forEach((hospital) => {
            updatehos([hospital.display_name.split(',')]);
  ...
  <div className="card shadow-lg slit-in-vertical" style={{height:"300px",width:"300px",backgroundColor:"rgba(93,117,126,255)" ,color:"aliceblue"}}>
    ...
    <div className="card-body">
      ...
      <a href="#" class="btn" style={{textDecoration:"none",border:"none",borderRadius:"50px",color:"aliceblue",backgroundColor:"rgba(66,77,92,255)"}}>Go somewhere</a>
    </div>
  </div>
</div>
))}
