...
def diet():
    ...
    {context}
        ...
        NOTE FOR LLM:don't use ** and * in the  diet plan
        ...
        food peference: {type}(refers to region the users want their food from)
        vegetarian: {og}(if yes only use vegetarian meals)
        Note for LLM:dont forget meals and their ingredient section.
        {input}                                      
 """)
    document_chain = create_stuff_documents_chain(llm, prompt)
    ...
    retrieval_chain = create_retrieval_chain(retriever, document_chain)
    response = retrieval_chain.invoke({'input':"please generated the diet following the template given in context",'h':height1,'w':weight1,'m':bmi,'risk':keywords,'type':peference,'og':veg})
